...
{
	vec2 InverseRes = 1.0 / uResolution.xy;
	...
	vec3 Texel0 = texture2D(uImage0, C0).rgb;
	vec3 Texel1 = texture2D(uImage0, C1).rgb;
	vec3 Texel2 = texture2D(uImage0, C2).rgb;
	vec3 Texel3 = texture2D(uImage0, C3).rgb;
	
	// Calculated the expected time of the sample.
	vec4 T = A * Cy * vec4(uResolution.x) * Two + B + Cx;
...
vec4 NTSCCodec(vec2 UV)
{
	...
	vec4 IAccum = Zero;
	vec4 QAccum = Zero;
	float QuadXSize = uResolution.x * 4.0;
	...
	for(float n = -41.0; n < 42.0; n += 4.0)
	{
		...
		vec4 C = CompositeSample(vTexCoord) * CRange + MinC;
		vec4 WT = W * (CoordX  + A * CoordY * Two * uResolution.x + B);
